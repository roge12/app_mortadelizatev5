...
function loadImage(){
    ...
    image.onload = function (event) {
		...
		container.on("pressmove", function(evt) {
			...
			if((moveX + userPhoto.getTransformedBounds().width >= 498) && (moveX <= 0)){

				console.log("moviendo x" + moveX);
				console.log("tamaño x" + userPhoto.getTransformedBounds().width);
				userPhoto.x = moveX;
			}
			//if((moveY + userPhoto.getTransformedBounds().height >= 1) && (moveY <= 0)){
			if((moveY + userPhoto.getTransformedBounds().height >= 353) && (moveY <= 0)){
				userPhoto.y = moveY;
				console.log("moviendo y" + moveY);
...
function Menoszoom(){	
	//scaleComponent(userPhoto, container);
		console.log(userPhoto.getTransformedBounds().width);

	if (userPhoto.getTransformedBounds().width > 498){

	factor -= 0.1;

	console.log("haciendo zoom: "+factor);

	//userPhoto.regX = userPhoto.x 
	//userPhoto.regY = userPhoto.y
	var oldWidth = userPhoto.getTransformedBounds().width
	var oldHeight = userPhoto.getTransformedBounds().height
	userPhoto.scaleX = factor;
	userPhoto.scaleY = factor;
	var newWidth = userPhoto.getTransformedBounds().width
	var newHeight = userPhoto.getTransformedBounds().height
	userPhoto.x = userPhoto.x - ((newWidth - oldWidth)/2);
	userPhoto.y = userPhoto.y - ((newHeight - oldHeight)/2);
